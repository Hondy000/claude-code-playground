...
class CalculatorGUI:
    ...
    def create_widgets(self):
        ...
        for (text, row, col) in buttons:
            btn = tk.Button(main_frame, text=text, font=('Arial', 18),
            ...
            if text.isdigit() or text == '.':
                ...
                btn.config(bg='#ff9500', fg='white', activebackground='#ff7700',
                         command=lambda t=text: self.on_operation(t))
            elif text == '=':
                btn.config(bg='#4cd964', fg='white', activebackground='#3cb954',
                         command=self.on_equals)
            elif text == 'C':
                btn.config(bg='#ff3b30', fg='white', activebackground='#dd2b20',
    ...
    def on_equals(self):
        """=が押されたときの処理"""
        if self.pending_operation and self.pending_value is not None and self.current_input:
            self.calculate()
            self.pending_operation = None
            self.pending_value = None
