...
async function runWorkerDemo() {
  ...
  // CPUコア数を取得
  const cpuCount = config.worker.poolSize || cpus().length || 4;
  ...
  try {
    ...
    console.time('並列処理');
    const primePromises = ranges.map((range) => pool.execute({ type: 'findPrimes', ...range }));
    ...
    const fibPromises = fibNumbers.map((n) => pool.execute({ type: 'fibonacci', n }));

    const fibResults = await Promise.all(fibPromises);
    fibResults.forEach((r, i) => {
      const value = r.result;
      console.log(
        `F(${fibNumbers[i]}) = ${value.slice(0, 20)}... (${value.length}桁, ${r.duration.toFixed(2)}ms)`,
      );
    });
    ...
    const totalPixels = mandelbrotResults.reduce((sum, r) => sum + r.result.length, 0);
